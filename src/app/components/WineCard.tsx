...
import { buildAromaLabelIndex, AROMA_CATEGORIES } from "../../data/aromas";
...
export function WineCard({ name, type, vintage, region, composition, variety, blend, aromas, rating, onRate, onDelete }: WineCardProps) {
  const aromaLabels = buildAromaLabelIndex(AROMA_CATEGORIES);
        ...
        <div className="space-y-2">
          ...
          {aromas && aromas.length > 0 ? (
            <div className="flex flex-wrap gap-1">
              {aromas.slice(0, 5).map((aromaId) => (
                <Badge 
                  key={aromaId} 
                  variant="outline" 
                  className="text-[10px] bg-amber-50 border-amber-900/20 text-amber-900"
                >
                  {aromaLabels[aromaId] || aromaId}
                </Badge>
              ))}
              {aromas.length > 5 && (
                <Badge variant="outline" className="text-[10px] bg-amber-50 border-amber-900/20 text-amber-700">
                  +{aromas.length - 5}
                </Badge>
              )}
            </div>
